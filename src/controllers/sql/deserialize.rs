// Code from https://github.com/kurtbuilds/sqlx-pgrow-serde
// Commit c56e7bfe37cce9d8ead83ca13c06b59aa29af10e
...
use sqlx::postgres::{PgValueRef, Postgres}; // 1.0.104
...
/* Not part of original library. Deserializes a single column json value */
pub fn read_json_row(row: &PgRow) -> Value {
    let columns = row.columns();
    if columns.len() != 1 {
        return Value::Null;
    }
    let value = row.try_get_raw(columns[0].ordinal()).unwrap();
    let value = SerPgValueRef(value);
    let value = serde_json::to_value(&value).unwrap();
    value
}
...
#[derive(Serialize)]
pub struct SerPgValueRef<'r>(#[serde(serialize_with = "serialize_pgvalueref")] PgValueRef<'r>);
